...
Wir haben es eingerichtet, dass eine Versionsdatei von GitHub geladen wird. Wenige Bytes überprüfen, ob eine aktuellere Version des Spiels vorliegt und machen entsprechende Meldung im Spiel.
...
von absoluten Bildschirmkoordinaten in Fenster/Spielkoordinaten umgerechnet werden. Diesen Fehler konnte wir nach einigem ausprobieren und rechnen beheben.
...
Position. So stehen die Mauer oder auch die Pfeile dort vermerkt. Es steht auch zu welchem Level gewechselt werden soll, wenn 
...
das Ganze in ein irres Durcheinenader. Zuerst brauchen wir die Anzahl Elemente, die es hat, um zu erkennen welche Information 
zu welchem Objekt gehört. Denn das Programm liest dort nur Zahlen heraus. Mit Wörtern können wir ihm das nicht erklären, 
da er diese nicht versteht.
...
neu geladen. So entsteht das Gefühl einer Bewegung oder hier eine Anpassung der Farbe. Entscheidend ist dabei die Methode
$animationAusfuehren()$, die überprüft, ob bereits genug Zeit vergangen ist, um das neue Bild anzuzeigen. Gegebenenfalls
übernimmt sie diese Aufgabe, es anzuzeigen.


\subsection{Pfeile}
Zuerst haben wir geplant die Pfeile nur als Anzeiger zu nutzen, um Eingänge zu kennzeichnen. Doch dann hatten wir die Idee, dass man sie gleich als Teleporter benutzen könnte. So standen wir vor einer viel schwereren Herausforderung, als zuerst angenommen. Um das zu lösen, geben wir in der Leveldatei das zu ladende Level an. Stösst man auf einen Pfeil wird das herausgelesen und man wechselt das Level. Wir geben auch noch ganz normal die Anzahl Pfeile an. Darauf folgt ihr momentaner Standort mit welcher Rotation. Die zwei zusätzlichen Zahlen geben die neue Starposition der Figur im neuen Level an. Dies können wir später benutzen, um Häuser mit mehreren Etagen zu bilden. Die rote Pfeile kennzeichnen die Ausgänge des Hauses. Durch eine weitere Zahl in der Leveldatei wird angegeben, welche Pfeile geladen werden müssen.
Konkret mussten wir diese Teleportfähigkeit einbauen, damit wir als Einbrecher von dem Garten ins Haus einbrechen können. Das bedeutet auch, dass wir pro Level am Ende zwei Leveldateien haben. Eine Leveldatei für das Innere des Hauses und eine für den Garten.

\subsection{Console}
Wir haben eine Console eingebaut, die sich mit der rechten Shift-Taste öffnen lässt. In dieser kann man verschiedene kleine Hilfen aktivieren. Wie zum Beispiel, dass man durch Mauern und Türen gehen kann. Man kann das Level direkt wechseln.
