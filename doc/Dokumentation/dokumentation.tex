...
ihre Zeit mit Computer- und Konsolenspielen.
...
\\
Viele wollen nur wissen, wie man ein Spiel spielt und nicht wie man es entwickelt. Nur wenige wissen, wie schwer und aufwändig es ist, bereits ein kleines Spiel zu machen.
...
Jeder von uns hatte bereits ein wenig Erfahrung damit gesammelt.
Es hat jedoch noch keiner alleine
ein ganzes Game programmiert.
...
\\
Wir möchten hier auch gleich die Gelegenheit nutzen und unserem Freund Mathias Rassasse danken. Er war ursprünglich auch in unsere Gruppe dabei. Er verliess dann jedoch die Schule. Wir möchten ihm danken, weil er für uns extra die gesamte Musik von unserem Game komponierte und einspielte.
\\
Wir bedanken uns auch bei unserem Mentor, Herrn Sax. Er hat uns immer konstruktive Rückmeldungen und wertvolle Tipps gegeben.
...
Programmiersprache C++ zu lernen und zu vertiefen. Dies wollen wir nach dem Prinzip Learning-by-Doing erreichen. Jeder in der Gruppe sollte am Ende
...
mit dem Versionskontrollsystem git (Verweis) vertraut werden. Das soll uns später
auf andere Projekte und unsere Maturarbeit vorbereiten.
\item
...
Als Einbrecher versucht man, in ein Haus einzubrechen und dabei keinen Lärm zu verursachen, Laserschranken auszuweichen. Je nach Spielmodus hat man eine andere Aufgabe zu bewältigen.

\section{Spielmodi}
Wir haben unterschiedliche Aufbauten der Levels vorgesehen:
\begin{itemize}
\item Aufgabe: Alle versteckten Schätze zu finden und einzusammeln.
\item Zeitdruck: Wir implementierten eine Zeitlimite (Siehe auch Abschnitt \ref{zeitlimite}), die zur  Aufgabenerfüllung nicht überschritten werden darf. Die Zeit ist bewusst sehr knapp gewählt, um den Spieler unter Druck zu setzen.
...
Diese Module wurden plattformübergreifend entwickelt. Deshalb mussten wir uns nicht um die Portierung umseres Spiels auf verschiedene Plattformen kümmern.
\\
\\
...
anschliessend in eine ausführbare Datei zusammenzuhängen (\q{linken}).

\section{git und GitHub}
\textbf{git} ist eine verteilte Dokumenten-Versionskontrolle (DVCS\footnote{Distributed Version Control System}). Ein solches Werkzeug hilft
den Entwicklern parallel zu programmieren und dabei die Übersicht zu behalten. Es zeichnet alle Änderungen am Code auf.  Diese kann man einsehen und zurückverfolgen, um Fehler zu finden und zu korrigieren. Sie unterstützen auch die Wiederherstellung früherer Programmabschnitten, ohne dass man
manuell die Dateien zerpflücken muss. 
\textbf{Git} überzeugte uns durch seine Einfachheit, seine Effizienz und seine Möglichkeiten.
\textbf{Git} erwies sich als grosse Hilfe, sei es um Fehler zu finden oder einfach zum synchronisieren des Codes.
\\
\\
\textbf{GitHub} ist ein
Onlineportal, das das Hosting, also das Speichern von Projekten, die mit \textbf{git} verwaltet werden, kostenlos übernimmt. Das Projekt erhält eine eigene Übersichtsseite,
die ebenfalls zur Entwicklung genutzt werden kann und auf der (fremde) Interessierte den Code und die Änderungen einsehen können. Wir haben \textbf{GitHub} nicht nur
...
Das führte schlussendlich zu einem Speicherleck von knapp 20MB. Dies konnte dank \textbf{Valgrind} rasch festgestellt und behoben werden konnte.
...
Disposition vermerkt - intensiv mit der Programmiersprache C++. Das Einarbeiten in die Sprache war zentral für die spätere Verwendung der SFML-Bibliothek. In die SFML Bibliothek arbeiteten wir uns mithilfe der offiziellen Tutorials \cite{sfml_tutorials} ebenfalls sehr anwendungsorientiert ein.
\\
...
//
Bei diesen Tastenabfragen wird auch gleich überprüft, ob man in eine Mauer läuft, sollte dies nämlich der Fall sein, darf sich die Figur nicht bewegen (Siehe Kollisionsdetektion (Referenz)). 
...
Die ist die Struktur, die wir implementierten.\\
\begin{figure}[h]
\centering
...
\\
\textbf{InputLoop}
Dieser Teil ist dafür zuständig, die Eingaben des Spielers einzulesen und aufgrund der Eingaben Aktionen auszulösen. 
Wenn zum Beispiel der Spieler in der Nähe eines Schatzes steht, und die Taste \key{E} drückt, werden dem Spieler 10 Punkte gutgeschrieben und der Schatz muss aus der 
RenderLoop entfernt werden, damit er beim nächsten Durchgang im InputLoop verschwindet. Steht der Spieler bei einer Türe und drückt \key{E}, wird die Animation
der Türe gestartet und die Mauer der Türe muss umplatziert werden. Wenn sich der Spieler in den Strahl einer Laserschranke stellt, wird das Level
\q{Game Over} geladen.\\
\\
\textbf{AnimationLoop}
...
ohne Ruckler spielbar wäre, hatten wir unsere Spielarchitektur ursprünglich nur seriell geplant:
\\
\begin{figure}[h]
\centering
